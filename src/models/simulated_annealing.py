...
class SimulatedAnnealing():
    ...
    """
        nodos = df_fv.nodoA.unique().tolist()
        combinaciones = [[a, b] for a in nodos for b in nodos]
        df2 = pd.DataFrame(combinaciones,columns=['nodoA', 'nodoB'])
        df2['distancia'] = 0
        i=0
        for row in df2.values:
            if (row[0]==row[1]):
                ...
                v= df_fv.query('nodoA == {} & nodoB == {}'.format(row[0],row[1]))
                v2=v['distancia'].tolist()
                row[2]= v2[0]
            elif (row[0]!=row[1] and row[0]>row[1]):
                v= df_fv.query('nodoA == {} & nodoB == {}'.format(row[1],row[0]))
                v2= v['distancia'].tolist()
                row[2]= v2[0]
            df2.iloc[i]=row
            i+=1
        dic = {k : g.distancia.to_dict() for k, g in df2.set_index('nodoA').groupby('nodoB')}
